...
selected_dataset = st.selectbox("Select a dataset for modeling", artifact_datasets, format_func=lambda x: x.name)
...
if selected_dataset:
    ...
    selected_input_feature_names = st.multiselect("Select input features (multiple)", feature_names)
    selected_target_feature_name = st.selectbox("Select target feature", feature_names)
    
    selected_input_features = [feature for feature in features if feature.name in selected_input_feature_names]
    selected_target_feature = next(feature for feature in features if feature.name == selected_target_feature_name)
...
if st.button("Train!"):
    ...
    train_df = pd.DataFrame(results['train_metrics'], columns=["Metric", "Training Score"])
