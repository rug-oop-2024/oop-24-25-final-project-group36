...
class MultipleLinearRegression(Model):
    ...
    def __init__(self, hyperparameters: dict = None):
        """
        Initialize the MultipleLinearRegression model with required fields for the Artifact.

        Args:
            hyperparameters (dict): Hyperparameters for the model, passed to scikit-learn's LinearRegression.
        """
        super().__init__(asset_path="multiple_linear_regression",
                         data=b"",
                         version="1.0.0",
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        """
        Train the model using the given training data by fitting scikit-learn's LinearRegression model.
